...
export const isAddress = async (address: string) => {
    try {
        return await window.web3Provider.utils.isAddress(address);
    } catch (error) {
        debugError(error);
        return false;
    }
...
export const loadWeb3 = async (rpc: string) => {
    try {
        if(window.ethereum) window.web3Provider = new Web3(window.ethereum);
        else window.web3Provider = new Web3(rpc || window.bl_rpc || 'https://rpc.ankr.com/eth');
        web3Ready_.set(true);
    } catch (error) {
        debugError(error);
    }
...
export const loadContract = async (abi: any, address: string) => {
    return await new window.web3Provider.eth.Contract(abi, address);
}
...
export const getTransactionObject = async (args: GetTransactionObject) => {
    ...
    let txObj;
    try {
        if (!window.web3Provider) await loadWeb3(window.bl_rpc);
...
export const getBalance = async (address: string) => {
    if (!window.web3Provider) await loadWeb3(window.bl_rpc);
    return await window.web3Provider.eth.getBalance(address)
    .then((data) => {
        return data;
    })
    .catch((err) => {
